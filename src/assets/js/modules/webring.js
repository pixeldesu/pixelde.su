...
if (onionrings.length > 0) {
  onionrings.forEach((onionringElement) => {
    ...
    const randomLink = onionringElement.querySelector(
      "[data-webring-link-random]",
    );

    const siteIndex = onionringData.sites.indexOf("https://pixelde.su");
    const prevSite = onionringData.sites[siteIndex - 1];
    const nextSite = (siteIndex + 1) === onionringData.sites.length
      ? onionringData.sites[0]
      : onionringData.sites[siteIndex + 1];
